...
export default class WordForm extends React.Component {
  ...
  buildBoardObject = () => {
    const letters = this.props.letters.toLowerCase()
    if (letters[0] !== " ") {
    ...
    return (
      <div className="input-form container">
        <form onSubmit={this.handleSubmit} >
          <input id="txtInputWord" type='text' onChange={this.handleChange} value={this.state.input} disabled={!this.props.isGameStarted} />
          <button type='submit' id='submitBtn' className="btn btn-sm enter-button" disabled={!this.props.isGameStarted} >Enter</button>
        </form>
